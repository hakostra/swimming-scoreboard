...
try:
    from .utils import parse_to_centis
except ImportError:  # Allow running `python server.py` from the package folder.
    from utils import parse_to_centis


# When running from source, `server.py`, `static/`, and `templates/` all live
# under `scoreboard/`. When running from a PyInstaller bundle, `sys._MEIPASS`
# points at the extracted bundle root, so we resolve into the packaged
# `scoreboard/` folder there as well.
if hasattr(sys, "_MEIPASS"):
    BASE_DIR = Path(sys._MEIPASS) / "scoreboard"
else:
...
async def _lifespan(app: FastAPI):
    ...
    try:
        ...
        # Terminate the comms helper process on shutdown, if running.
        _stop_comms_process()


app = FastAPI(title="Swimming Scoreboard Server", lifespan=_lifespan)
...
comms_process: subprocess.Popen | None = None
...
def _recompute_ranks_from_times() -> None:
    """Recompute lane ranks based on their time values."""

    lanes = scoreboard_state.get("lanes", [])
    parsed: list[tuple[int, int]] = []

    for idx, lane in enumerate(lanes):
        time_str = str(lane.get("time", "") or "").strip()
        if not time_str:
            lane["rank"] = ""
            continue
        try:
            centis = parse_to_centis(time_str)
        except (ValueError, TypeError):
            lane["rank"] = ""
            continue
        parsed.append((idx, centis))

    parsed.sort(key=lambda item: (item[1], lanes[item[0]].get("lane", 0)))

    current_rank = 0
    last_time: int | None = None
    for pos, (idx, centis) in enumerate(parsed, start=1):
        if last_time is None or centis != last_time:
            current_rank = pos
            last_time = centis
        lanes[idx]["rank"] = str(current_rank)
...
def _rebuild_lanes(first_lane: int, lane_count: int) -> None:
    ...
    existing = {lane.get("lane"): lane for lane in
                scoreboard_state.get("lanes", [])}
...
def _apply_lanes_payload(lanes_payload: list[Dict[str, Any]]) -> None:
    """Apply a lanes payload to the scoreboard state.
...
def _start_comms_process() -> None:
    """Start the external comms helper process using subprocess.
    ...
    try:
        if comms_process is not None and comms_process.poll() is None:
            return
    except Exception:
        # If polling fails for any reason, we'll try to start a fresh
        # process.
        pass
    ...
    try:
        ...
        if getattr(sys, "frozen", False):
            # In a bundled build, launch the same executable in comms mode.
            cmd = [sys.executable, "--comms"]
            cwd = str(Path(sys.executable).resolve().parent)
            env = os.environ.copy()
        else:
            # From source: run the comms module in-package.
            ...
            if root_str not in existing_pp.split(os.pathsep):
                env["PYTHONPATH"] = (
                    root_str if not existing_pp else
                    f"{root_str}{os.pathsep}{existing_pp}"
                )

            cmd = [sys.executable, "-m", "scoreboard.comms"]
...
async def update_lanes(payload: Dict[str, Any]):
    ...
    if isinstance(lanes_payload, list):
        _apply_lanes_payload(lanes_payload)
        await manager.broadcast(scoreboard_state)
    return {"status": "ok"}
...
async def start_timer(payload: Dict[str, Any] = Body(default=None)):
...
async def sort_by_rank():
    ...
    _recompute_ranks_from_times()
    scoreboard_state["sort_mode"] = "rank"
    await manager.broadcast(scoreboard_state)
    return {"status": "ok", "sort_mode": "rank"}


@app.post("/api/prepare_heat")
async def prepare_heat(payload: Dict[str, Any]):
    """Prepare a new heat in one call: sort by lane, reset timer,
    set header, and set lanes."""
...
@app.post("/api/finish_heat")
async def finish_heat():
    """Finish the current heat by clearing unfinished lanes,
    resetting the timer, and sorting by rank."""

    lanes = scoreboard_state.get("lanes", [])
    for lane in lanes:
        finished = _coerce_finished(lane.get("finished", False))
        if not finished:
            lane["name"] = ""
            lane["rank"] = ""
            lane["time"] = ""
...
async def update_timing_config(payload: Dict[str, Any]):
    ...
    if debug_enabled_raw is not None:
        timing_config["debug_capture_enabled"] = \
            _coerce_bool(debug_enabled_raw)
        changed = True
